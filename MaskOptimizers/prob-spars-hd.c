...
printf("\n");
printf("HOMOGENEOUS DIFFUSION INPAINTING MASK OPTIMISATION\n");
...
/* initialise inpainting u with original image f */
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
  for (m=0; m<=nc-1; m++)
      u[m][i][j] = f[m][i][j];
...
mse = MSE (nc, nx, ny, u, f);
mae = MAE (nc, nx, ny, u, f);
