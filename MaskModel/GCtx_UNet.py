...
class GlobalQueryGen(nn.Module):
    ...
    def forward(self, x):
        ...
        B = x.shape[0]
        # print(f"globalQueryGen : {x.shape}")
        # print(self.N,  self.num_heads, self.dim_head)
...
class GCViT(nn.Module):
    ...
    def __init__(self,
        ...
        depths2=[2, 2, 2, 1]#[3, 4, 19, 5]#depths
    ...
    def forward(self, x):
        # print(f"forward in GcVIt  : {x.shape}")
        x, x_downsample = self.forward_features(x)

        x = self.forward_up_features(x,x_downsample)
        x = self.up_x4(x)
        return x
class GCViT_Unet(nn.Module):
    """
    ...
    def __init__(self,config,
                 ...
                 attn_drop_rate=0.,
                 norm_layer=nn.LayerNorm,
                 layer_scale=None,#None,#1e-5 use this one for small
                 **kwargs):

        super(GCViT_Unet,self).__init__()
        self.num_classes = num_classes
        self.config = config
        self.resolution=img_size

        self.model = GCViT(dim=config.MODEL.GCVIT.EMBED_DIM,
                            depths=config.MODEL.GCVIT.DEPTHS,
                            window_size=config.MODEL.GCVIT.WINDOW_SIZE,
                            mlp_ratio=config.MODEL.GCVIT.MLP_RATIO,
                            num_heads=config.MODEL.GCVIT.NUM_HEADS,
                            resolution=self.resolution,
                            drop_path_rate=config.MODEL.DROP_PATH_RATE,
                            in_chans=config.MODEL.GCVIT.IN_CHANS,
    ...
    def forward(self, x):
        # print(f"forward in GcVIt Unet : {x.shape}")
        # if x.size()[1] == 1:
        #     x = x.repeat(1,3,1,1)
        logits = self.model(x)
    ...
    def load_from(self,config):
        ...
        if pretrained_path is not None:
            ...
            print("---start load pretrained modle of GC-ViT encoder---")
