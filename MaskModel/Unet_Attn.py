...
class up_conv(nn.Module):
    def __init__(self,ch_in,ch_out):
        ...
        self.up = nn.Sequential(
            ...
            nn.Conv2d(ch_in,ch_out,kernel_size=3,stride=1,padding=1,padding_mode = 'reflect', bias=True),
...
class conv_block(nn.Module):
    def __init__(self,ch_in,ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch_out, ch_out, kernel_size=3,stride=1,padding=1,padding_mode = 'reflect', bias=True),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
...
class AttU_Net(nn.Module):
    def __init__(self,img_ch=3,output_ch=1, tar_den = 0.1):
        ...
        self.Att2 = Attention_block(F_g=16,F_l=16,F_int=1)
    ...
    def forward(self,x):
        ...
        d1 = torch.special.expit(self.Conv_1x1(d2))
        prob = self.scaleDensity(d1)
