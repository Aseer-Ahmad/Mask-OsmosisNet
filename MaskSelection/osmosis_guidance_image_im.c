...
for (i=1; i<=nx; i++)
 for (j=1; j<=ny; j++)
   u[i][j] = ((u[i][j] - min) / (max- min)) * (new_max - new_min) + new_min;
...
/* analyse guidance image */
printf ("guidance image\n");
analyse_grey_double (v, nx, ny, &min, &max, &mean, &std);
printf ("minimum:              %8.2lf \n", min);
printf ("maximum:              %8.2lf \n", max);
printf ("mean:                 %8.2lf \n", mean);
printf ("standard dev.:        %8.2lf \n\n", std);
...
/* min max scaling of output and calculate metrics */
double gd_min, gd_max;
analyse_grey_double (v, nx, ny, &gd_min, &gd_max, &mean,  &std);
